...
def plot_smartphone_weartime(input_path: str) -> None:
    """Plot smartphone weartime.
    ...
    print("(3/3) Creating wear-time figures.../n")

    weekdays = np.array(["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"])

    weartime_files = os.listdir(os.path.join(output_path, "wear_time"))

    # create / clear figures
    plt.figure(0, clear=True, figsize=(30, 24))  # average in 24h cycle
    plt.figure(1, clear=True, figsize=(30, 24))  # cum. over duration of study
    plt.figure(2, clear=True, figsize=(30, 24))  # average by weekdays
    for k, file in enumerate(weartime_files):
        subject = file.split("_")[0]

        # load data
        data = pd.read_csv(os.path.join(input_path, file)).to_numpy()
        timeline = data[:, 1]
        col_data = data[:, 2]
        wear_time = data[:, 3]

        # adjust wear_time data to col_data % proportion
        wear_adj = [i/j if j > 0 else 0 for i, j in zip(wear_time, col_data)]

        # time (days) vs. estimated weartime per day (1440 minutes)
        a = wear_adj
        win_size = 24*60
        new_shape = ((int(len(wear_adj)/win_size), win_size))
        ...
        # time of observation (24h) vs. wear-time per hour (in minutes)
        b = wear_adj

        win_size = 60
        new_shape = ((int(len(b)/win_size), win_size))
        ...
        c = np.reshape(b, new_shape)
        c = np.mean(c, axis=0)

        c_mean = np.mean(c)

        # over duration of a study
        plt.figure(0)
        plt.subplot(6, 8, k+1)
        plt.plot(a, 'o-', fillstyle="full",
                 color=(0.4, 0.4, 0.4), linewidth=0.5,
                 markersize=1, markeredgewidth=1,
                 markeredgecolor=(0.4, 0.4, 0.4))
        plt.text(len(a)-1, 1440,
                 "{:.0f}".format(a_mean),
                 fontsize=10, ha='right', va='top')
        plt.yticks(np.arange(0, 1441, 6*60))
        plt.ylim(-100, 1500)
        plt.xlabel("Day of observation (days)")
        ...
        plt.yticks(np.arange(0, 61, 30))
        plt.xlabel("Time of the day (hour)")
        plt.ylabel("Mean time (minutes)")
        plt.title(subject)

        plt.figure(2)  # average by weekdays
        # process timeline to match weekdays
        timeline = [datetime.fromisoformat(t) for t in timeline]
        t_ind = np.array([range(0, len(timeline), 24*60)[i]
                          for i in range(0, int(len(timeline)/24/60))])
        timeline = [timeline[i] for i in t_ind]

        # convert to weekdays
        timeline = np.array([t.weekday() for t in timeline])-1
        timeline = weekdays[timeline]

        # create weekly timeline
        try:
            weekly_average = [np.mean(a[np.array(
                [i for i in range(len(timeline)) if timeline[i] == day])])
                for day in weekdays]
            weekly_timeline = weekdays
        except IndexError:  # error raised when timeline is shorter than a week
            weekly_average = a
            weekly_timeline = timeline

        plt.subplot(6, 8, k+1)
        plt.plot(weekly_timeline, weekly_average, 'o-', fillstyle="full",
                 color=(0.4, 0.4, 0.4), linewidth=0.5,
                 markersize=1, markeredgewidth=1,
                 markeredgecolor=(0.4, 0.4, 0.4))

        # adjust plot limits
        if np.min(weekly_average) > 0:
            ylim_lo = np.min(weekly_average) - 10
        else:
            ylim_lo = -1
        ylim_hi = 1540

        plt.text(6, ylim_hi-5,
                 "{:.0f}".format(a_mean),
                 fontsize=10, ha='right', va='top')
        ...
        plt.yticks(np.arange(0, 1441, 6*60))
        plt.xlabel("Day of observation (weekday)")
        plt.ylabel("Mean cum. time (minutes)")
        plt.title(subject)
    ...
    print("/n(3/3) Figures plotted./n")


def estimate_and_plot_smartphone_weartime(input_path: str,
    ...
    plot_smartphone_weartime(input_path)
...
estimate_and_plot_smartphone_weartime(input_path, output_path)
